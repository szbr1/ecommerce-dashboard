...
function getRandomDate(
  ...
  if (monthsRange) {
    ...
    const month =
      Math.floor(Math.random() * (monthsRange.end - monthsRange.start + 1)) +
      monthsRange.start;
    date.setMonth(month);
  }
  return date;
...
async function main() {
  ...
  await prisma.$executeRawUnsafe(
    `TRUNCATE TABLE "Comment", "Review", "OrderItem", "Order", "ProductPriceHistory", "Product", "Category", "StoreAccess", "Follower", "StoreProfile", "Store", "Contact", "Address", "UserProfile", "manyUsers", "Role", "User", "StoreCounts" RESTART IDENTITY CASCADE;`
  );
  ...
  const ownerProfile = await prisma.userProfile.create({
    data: {
      userId: owner.id,
      imageUrl: 'https://i.pravatar.cc/150?u=owner',
      description: 'Lead Developer & Store Owner',
    },
  });
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    const year = isSpecialFilterRange
      ? 2025
      : Math.floor(Math.random() * (2025 - 2020 + 1)) + 2020;
    const orderDate = getRandomDate(
      year,
      year,
      isSpecialFilterRange ? { start: 0, end: 1 } : undefined
    );
